...
def parse_args():
    ...
    parser.add_argument('--dataset', nargs='?', default='cvr',
    ...
    parser.add_argument('--lamda', type=float, default=0.002,
...
class NeuralFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            if self.loss_type == 'square_loss':
                ...
                if self.lamda_bilinear > 0:
                    self.loss = tf.contrib.losses.log_loss(self.out, self.train_labels,  epsilon=1e-07, scope=None) + tf.contrib.layers.l2_regularizer(self.lamda_bilinear)(self.weights['feature_embeddings'])  # regulizer
                else:
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        for epoch in range(self.epoch):
            ...
            self.predict(Test_data, epoch+1)
    ...
    def predict(self, data, epoch):
        ...
        fw = open('./data/cvr/res/'+'submit'+str(epoch)+'.txt', 'a')
        fw.write('{} {}\n'.format('instance_id', 'predicted_score'))
        fr = open('./data/cvr/round1_ijcai_18_test_a_20180301.txt', 'r')
        fr.readline()
        for item in y_pred:

...
class LoadData(object):
    ...
    def __init__(self, path, dataset, loss_type):
        ...
        self.trainfile = self.path + dataset +".train_data.txt"
        self.testfile = self.path + dataset + ".test_data.txt"
        self.validationfile = self.path + dataset + ".val_data.txt"
    ...
    #     while line:
    #         items = line.strip().split(' ')
    #         X_.append([self.features[item] for item in items[0:]])
    #         line = f.readline()
    #     f.close()
    #     return X_
    ...
    # def construct_testdataset(self,X_):
    #     Data_Dic = {}
